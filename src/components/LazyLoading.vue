<template>
  <div class="w-screen h-[100vh] flex justify-center items-center">
    <div class="text-[72px] font-semibold items-center justify-center flex flex-col">
      <h1 class="mb-[24px]">Arco</h1>
      <svg width="72" height="72" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M24 3.99994V11.9999"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M38.1421 9.85779L32.4852 15.5146"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path d="M44 23.9999H36" stroke="#165DFF" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
        <path
          d="M38.1421 38.1421L32.4852 32.4852"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M24 43.9999V35.9999"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M9.85791 38.1421L15.5148 32.4852"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path d="M4 23.9999H12" stroke="#165DFF" stroke-width="4" stroke-linecap="round" stroke-linejoin="round" />
        <path
          d="M9.85791 9.85779L15.5148 15.5146"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M16.3467 5.5224L17.8774 9.21792"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M5.52246 16.3461L9.21798 17.8769"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M5.52246 31.6537L9.21798 30.123"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M16.3467 42.4777L17.8774 38.7822"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M31.6538 42.4777L30.123 38.7822"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M42.4777 31.6537L38.7822 30.123"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M42.4777 16.3461L38.7822 17.8769"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
        <path
          d="M31.6538 5.5224L30.123 9.21792"
          stroke="#165DFF"
          stroke-width="4"
          stroke-linecap="round"
          stroke-linejoin="round"
        />
      </svg>
    </div>
  </div>
</template>
