<template>
  <div class="h-[80vh] w-[100%] flex flex-col justify-center items-center">
    <ul
      v-if="collect.data.rows[0] !== undefined"
      class="w-[100%]"
      :class="collect.data[currentPage - 1] === collect.data[collect.data.length - 1] ? 'h-[75vh]' : ''"
    >
      <li
        v-for="(item, index) in collect.data.rows[currentPage - 1]"
        :key="index"
        ...
        <div class="flex justify-between">
          ...
          <div class="w-[160px] flex">
            <el-tooltip class="box-item" effect="dark" content="项目成员" placement="top-start">
              <el-button :icon="User" circle @click="getUserlist(item.id)"> </el-button>
            </el-tooltip>
...
const getlist = () => {
  projectApi
    ...
    .then((res: any) => {
      collect.value.data = res.data;
      collect.value.data.rows = groupArray(res.data.rows.reverse(), 11);
      collect.value.code = res.code;
      collect.value.msg = res.msg;
      console.log(collect.value.data.rows);
    })
    .catch((err) => {
      console.log(err);
    });
